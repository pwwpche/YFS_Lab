/*
 * test-lab-6-a /classfs/dir1 /classfs/dir2
 *
...
create1(const char *d, const char *f, const char *in)
{
  ...
  fd = creat(n, 0666);
  if(fd < 0){
    fprintf(stderr, "test-lab-6-a: create(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(write(fd, in, strlen(in)) != strlen(in)){
    fprintf(stderr, "test-lab-6-a: write(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(close(fd) != 0){
    fprintf(stderr, "test-lab-6-a: close(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
check1(const char *d, const char *f, const char *in)
{
  ...
  fd = open(n, 0);
  if(fd < 0){
    fprintf(stderr, "test-lab-6-a: open(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  errno = 0;
  //fprintf(stderr, "test-lab-6-a: read(%s) reading fd=%d, sizeof(buf)-1=%d\n", fd, sizeof(buf) - 1);
  cc = read(fd, buf, sizeof(buf) - 1);
  //fprintf(stderr, "test-lab-6-a: read(%s) reading done, fd=%d, buf=%s\n", fd);
  if(cc != strlen(in)){
    fprintf(stderr, "test-lab-6-a: read(%s) returned too little %d%s%s\n",
            n,
            cc,
            errno ? ": " : "",
            errno ? strerror(errno) : "");
  fprintf(stderr, "test-lab-6-a: cc=%d, strlen(in)=%lu\n", cc, strlen(in));
    exit(1);
  }
  close(fd);
  buf[cc] = '\0';
  if(strncmp(buf, in, strlen(n)) != 0){
    fprintf(stderr, "test-lab-6-a: read(%s) got \"%s\", not \"%s\"\n",
            n, buf, in);
    exit(1);
  }
}
...
unlink1(const char *d, const char *f)
{
  ...
  sprintf(n, "%s/%s", d, f);
  if(unlink(n) != 0){
    fprintf(stderr, "test-lab-6-a: unlink(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
checknot(const char *d, const char *f)
{
  ...
  fd = open(n, 0);
  if(fd >= 0){
    fprintf(stderr, "test-lab-6-a: open(%s) succeeded for deleted file\n", n);
    exit(1);
  }
}
...
append1(const char *d, const char *f, const char *in)
{
  ...
  if(fd < 0){
    fprintf(stderr, "test-lab-6-a: append open(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(write(fd, in, strlen(in)) != strlen(in)){
    fprintf(stderr, "test-lab-6-a: append write(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
  if(close(fd) != 0){
    fprintf(stderr, "test-lab-6-a: append close(%s): %s\n",
            n, strerror(errno));
    exit(1);
  }
}
...
write1(const char *d, const char *f, int start, int n, char c)
{
  ...
  if(lseek(fd, start, 0) != (off_t) start){
    fprintf(stderr, "test-lab-6-a: lseek(%s, %d): %s\n",
  ...
  for(int i = 0; i < n; i++){
    ...
    if(fsync(fd) != 0){
      fprintf(stderr, "test-lab-6-a: fsync(%s): %s\n",
              name, strerror(errno));
      exit(1);
    }
  }
...
checkread(const char *d, const char *f, int start, int n, char c)
{
  ...
  for(int i = 0; i < n; i++){
    char xc;
    if(read(fd, &xc, 1) != 1){
      fprintf(stderr, "test-lab-6-a: read(%s): %s\n",
              name, strerror(errno));
      exit(1);
    }
    if(xc != c){
      fprintf(stderr, "test-lab-6-a: checkread off %d %02x != %02x\n",
              start + i, xc, c);
      exit(1);
    }
  }
...
checkn(const char *d, const char *prefix, int nf)
{
  ...
  for(i = 0; i < nf; i++){
    ...
    j = -1;
    //tfprintf("checkn");
    //fprintf(stderr, "test-lab-6-a: read() reading fd=%d, sizeof(j)=%lu\n", fd, sizeof(j));
    //fflush(stderr);
    cc = read(fd, &j, sizeof(j));
   // tfprintf("checkn");
    //fprintf(stderr, "test-lab-6-a: read() reading done fd=%d, cc=%d, j=%d, i=%d, sizeof(j)=%lu\n", fd,cc,  j,i,  sizeof(j));
    //fflush(stderr);
    if(cc != sizeof(j)){
      tfprintf("checkn");
      fprintf(stderr, "test-lab-6-a: read(%s) returned too little %d%s%s\n",
              ...
              errno ? ": " : "",
              errno ? strerror(errno) : "");
	  fprintf(stderr, "test-lab-6-a: cc=%d, j=%d, sizeof(j)=%lu\n", cc, j, sizeof(j));
      fflush(stderr);
      //exit(1);
    }
    if(j != i){
      fprintf(stderr, "test-lab-6-a: checkn %s contained %d not %d\n",
              n, j, i);
      //exit(1);
    }
    close(fd);
...
dircheck(const char *d, int nf)
{
  ...
  dp = opendir(d);
  if(dp == 0){
    fprintf(stderr, "test-lab-6-a: opendir(%s): %s\n", d, strerror(errno));
    exit(1);
  }
  while((e = readdir(dp))){
    if(e->d_name[0] != '.'){
  ...
  if(nf != nnames){
    fprintf(stderr, "test-lab-6-a: wanted %d dir entries, got %d\n", nf, nnames);
    exit(1);
  }
  ...
  for(i = 0; i < nnames-1; i++){
    if(strcmp(names[i], names[i+1]) == 0){
      fprintf(stderr, "test-lab-6-a: duplicate directory entry for %s\n", names[i]);
      exit(1);
    }
  }
...
main(int argc, char *argv[])
{
  ...
  if(argc != 3){
    fprintf(stderr, "Usage: test-lab-6-a dir1 dir2\n");
    exit(1);
  }

  sprintf(d1, "%s/d%d", argv[1], getpid());
  if(mkdir(d1, 0777) != 0){
    fprintf(stderr, "test-lab-6-a: failed: mkdir(%s): %s\n",
            d1, strerror(errno));
    exit(1);
  }
  sprintf(d2, "%s/d%d", argv[2], getpid());
  if(access(d2, 0) != 0){
    fprintf(stderr, "test-lab-6-a: failed: access(%s) after mkdir %s: %s\n",
  ...
  printf("test-lab-6-a: Passed all tests.\n");
