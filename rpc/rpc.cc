                                ...
                                unsigned int xid_rep, char **b, int *sz)
{
    ...
    ScopedLock rwl(&reply_window_m_);
	//tprintf("checkduplicate begin, xid=%u, client=%u, xid_rep=%u\n", xid, clt_nonce, xid_rep);
    //Indicating that the xid has been seen but deleted
    bool forgetten = true;

    if(reply_window_.find(clt_nonce) == reply_window_.end())
    {
  		//tprintf("checkduplicate, client=%u not found\n",clt_nonce);
        return FORGOTTEN;
    }
    //tprintf("checkduplicate found,client=%u\n",clt_nonce);
    //NEW: never seen this xid before, remembers the request in reply_window_.
    if(reply_window_[clt_nonce].empty())
    {
        //tprintf("checkduplicate empty, client=%u\n",clt_nonce);
        reply_t reply(xid);
        reply_window_[clt_nonce].push_back(reply);
        //tprintf("checkduplicate done with NEW, client=%u\n",clt_nonce);
        return NEW;
    }

    //tprintf("checkduplicate start finding, xid=%u, client=%u, xid_rep=%u\n", xid, clt_nonce, xid_rep);
    std::list<reply_t>::iterator it;
    for(it = reply_window_[clt_nonce].begin() ; it != reply_window_[clt_nonce].end() ; )
    {
        //tprintf("checkduplicate finding, current=%u, xid_rep=%u\n",it->xid, xid_rep);
        // deletes remembered requests with XIDs <= xid_rep;
        if(it->xid < xid_rep)
        {
            //tprintf("checkduplicate erase, current=%u, xid=%u\n",it->xid, xid);
            free(it->buf);
            ...
            //tprintf("checkduplicate current, current=%u, xid=%u\n",it->xid, xid);
            if(it->xid == xid)
            {
                if(it->cb_present == true)
                {
                    *b = it->buf;
                    *sz = it->sz;
                    //tprintf("checkduplicate done with DONE, client=%u\n",clt_nonce);
                    return DONE;
                }
                else
                {
                    //tprintf("checkduplicate done with INPROGRESS, client=%u\n",clt_nonce);
                    return INPROGRESS;
                }
            }
            //The xid has been seen and exists
            if(it->xid < xid)
            {
                //tprintf("checkduplicate forgotten, current=%u, xid=%u\n",it->xid, xid);
                forgetten = false;
            }
    ...
    //tprintf("checkduplicate end finding, xid=%u, client=%u, xid_rep=%u\n", xid, clt_nonce, xid_rep);
    //Re-add existing reply to window
    if(forgetten == false)
    {
        reply_t reply(xid);
        reply_window_[clt_nonce].push_back(reply);
        //tprintf("checkduplicate done with forgotten NEW, client=%u\n",clt_nonce);
        return NEW;
    }
    //tprintf("checkduplicate done with FORGOTTEN, client=%u\n",clt_nonce);
    return FORGOTTEN;
...
rpcs::add_reply(unsigned int clt_nonce, unsigned int xid, char *b, int sz)
{
	...
	//tprintf("add_reply begin, xid=%u, client=%u\n", xid, clt_nonce);
    //Check whether this client exists in the reply window
    if(reply_window_.find(clt_nonce) == reply_window_.end())
    {
    	//tprintf("add_reply client not exists\n");
        return ;
    }
    //Add reply content to reply_window_
    std::list<reply_t>::iterator it;
   	//tprintf("add_reply client start finding\n");
    for(it = reply_window_[clt_nonce].begin() ; it != reply_window_[clt_nonce].end() ; it++)
    {
   	   	//tprintf("add_reply client finding xid=%u, orig_xid=%u, clt=%u\n", it->xid, xid, clt_nonce);
        if(it->xid == xid)
        {
        	//tprintf("add_reply client finding xid=%u done\n", it->xid);
            //Modify properties
   	...
   	//tprintf("add_reply client ends finding, clt=%u\n", clt_nonce);
    return ;
}
