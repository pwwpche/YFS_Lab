...
void fuseserver_symlink( fuse_req_t req,const char *target,
    ...
    yfs_client::inum inum;
    yfs_client::status ret;
    std::string pathStr(target);
    printf("name=%s, target=%s\n", name, target);
    if( (ret = yfs->symlink(parent,name,target,inum)) == yfs_client::OK ) {
        e.ino = inum;
        getattr(inum, e.attr);
        fuse_reply_entry(req, &e);
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    yfs_client::syminfo info;
    if( (ret = yfs->getsym(ino, info) ) == yfs_client::OK)
    {
        std::string linkStr;
        if( (ret = yfs->read(ino, info.size, 0, linkStr)) == yfs_client::OK)
        {
            fuse_reply_readlink(req, linkStr.c_str());
            return ;
        }
    }
    fuse_reply_err(req, yfs_client::IOERR);
}
